...
async function loadDiscsFromSheets() {
  ...
  try {
    ...
    const discs = (Array.isArray(data) ? data : (data.discs || [])).filter(function (d) {
...
function buildImageCover(disc, cfg, platformLabel, statusText) {
  ...
  return `
  <div class="cover cover-img" style="background-image:url('${normalizeImageUrl(disc.imageUrl)}')">
...
function initPreorderModal(allDiscs) {
  ...
  function openModal(card) {
    ...
    if (coverEl && mCover) {
mCover.style.backgroundImage = coverEl.style.backgroundImage || '';
